...
export class StallsService {
  ...
  async create(createStallDto: CreateStallDto, ownerId: string): Promise<Stall> {
  ...
  async update(id: string, updateStallDto: UpdateStallDto, ownerId: string): Promise<Stall> {
    ...
    if (stall.status !== 'pendiente') {
      throw new BadRequestException('Solo se pueden modificar puestos en estado pendiente');
    }
  ...
  async changeStatus(id: string, changeStatusDto: ChangeStatusDto): Promise<Stall> {
    const stall = await this.findOne(id);

    // Validaciones de transición de estado
    if (stall.status === 'activo' && changeStatusDto.status !== 'activo') {
      throw new BadRequestException('No se puede cambiar el estado de un puesto activo');
    }
  ...
  async findByStatus(status: 'pendiente' | 'aprobado' | 'activo'): Promise<Stall[]> {
    return await this.stallsRepository.find({
      where: { status },
      order: { createdAt: 'DESC' }
    });
